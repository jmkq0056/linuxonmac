...
SHELL_RC="${HOME}/.config/devenv/shell.sh"
MAC_MOUNT=/mnt/mac
...
stage_shell() {
  ...
  mkdir -p "$(dirname "$SHELL_RC")" "$LOCAL_BIN"
...
# ---- PATH ----------------------------------------------------------------
_devenv_path_add() { case ":\$PATH:" in *":\$1:"*) ;; *) PATH="\$1:\$PATH" ;; esac; }

_devenv_path_add "\$HOME/.local/bin"
_devenv_path_add "$FNM_DIR"
# Stable path that follows fnm's "default" alias, so node/npm/npx/claude are
# available even in shells that never evaluate \`fnm env\`.
_devenv_path_add "$FNM_DIR/aliases/default/bin"
[ -d "$FLUTTER_DIR/bin" ] && _devenv_path_add "$FLUTTER_DIR/bin"
[ -d "\$HOME/.pub-cache/bin" ] && _devenv_path_add "\$HOME/.pub-cache/bin"
export PATH

${jhome:+export JAVA_HOME="$jhome"}
export FNM_DIR="$FNM_DIR"
export PNPM_HOME="\$HOME/.local/share/pnpm"
_devenv_path_add "\$PNPM_HOME"; export PATH
...
# Podman exposes a Docker-compatible API socket; point Docker-API clients
# (testcontainers, docker-compose, IDE plugins) at it.
[ -S "\${XDG_RUNTIME_DIR:-/run/user/\$(id -u)}/podman/podman.sock" ] && \\
  export DOCKER_HOST="unix://\${XDG_RUNTIME_DIR:-/run/user/\$(id -u)}/podman/podman.sock"

# Only the interactive bits from here down.
...
# ---- fnm (per-directory Node via .nvmrc / .node-version) ------------------
...
# containers — podman answers to \`docker\` too (podman-docker), these are for
# the compose/ps shapes people type most.
...
# ---- helpers -------------------------------------------------------------
# mongo-dev [start|stop|shell|logs] — a throwaway MongoDB 8 on localhost:27017
# with a named volume, so data survives \`stop\`.
mongo-dev() {
  local name=mongo-dev
  case "\${1:-start}" in
    start)
      podman container exists \$name && { podman start \$name; return; }
      podman run -d --name \$name -p 27017:27017 -v mongo-dev-data:/data/db \\
    ...
    shell)  command -v mongosh >/dev/null 2>&1 \\
...
# ---- prompt --------------------------------------------------------------
# user@host in the VM's own colour, cwd, and the git branch when there is one.
_devenv_git_branch() {
  local b
...
SHELL
  ...
  # Hook it into bash. The hook goes at the *top* of .bashrc, ahead of Debian's
  # "return early if not interactive" guard, so that `ssh guest 'npm ci'` and
  # systemd units get the PATH too — shell.sh has its own interactivity guard
  # and skips the aliases/prompt half by itself. .profile covers login shells
  # and the Plasma session's environment.
  local hook='# devenv (scripts/guest/10-dev-environment.sh) — must stay above the
# non-interactive early-return below; shell.sh guards its own interactive half.
[ -f "$HOME/.config/devenv/shell.sh" ] && . "$HOME/.config/devenv/shell.sh"
'
  prepend_once "${HOME}/.bashrc"  "$hook" '.config/devenv/shell.sh'
  append_once  "${HOME}/.profile" "$hook" '.config/devenv/shell.sh'

  info "shell config at $SHELL_RC"
  return 0
}
